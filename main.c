...
#include <stdlib.h>
#include <stdbool.h>
#include <sys/types.h> // Added to define ssize_t
#include <stdint.h>
#include <string.h>
#include <errno.h>
#include <fcntl.h>
...
const uint32_t LEAF_NODE_VALUE_OFFSET = LEAF_NODE_KEY_OFFSET +
                                        LEAF_NODE_KEY_SIZE;
const uint32_t LEAF_NODE_CELL_SIZE = LEAF_NODE_KEY_SIZE +
                                     LEAF_NODE_VALUE_SIZE;
const uint32_t LEAF_NODE_SPACE_FOR_CELLS = PAGE_SIZE -
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
     ...
     * Update parent or create a new paret.
     */
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
     ...
     * Starting from the right, move each key
     * to correct position.
     */
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num){
    ...
    /*
     * Declaring a flag before updating pointers which records whether this operation involves splitting
     * the root - if it does, we will insert our newly created node during the step where the tables's new
     * root is created. If it does not, we have to insert the newly created node into its parent after the
     * old node's keys have been transferred over. We are not able to do this if the newly created node's
     * parent is not a newly initialized root node, because in that case its parents may have existing keys
     * aside from our old node which we are splitting. If that is true, we need to find a place for our newly
     * created node in its parent, and we cannot insert it at the correct index if it does not yet have any keys
     */
    ...
    *internal_node_right_child(old_node) = INVALID_PAGE_NUM;
    ...
    /*
     * Set child before middle key, which is now the highest key, to be node's right child and
...
void internal_node_insert (Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    /* Add a new child/key pair to parent that
